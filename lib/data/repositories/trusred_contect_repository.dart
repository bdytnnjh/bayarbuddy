...
class TrustedContactRepository {
  ...
  Future<TrustedContactModel?> getTrustedContact(String contactId) async {
    final doc = await FirebaseQuery.getDocument(collection: _collection, docId: contactId);
  ...
  Future<List<TrustedContactModel>> getTrustedContactsByUserId(String userId) async {
    ...
    return snapshot.docs.map((doc) => TrustedContactModel.fromSnapshot(doc)).toList();
  }

  // Read - Get all trusted contacts (including inactive)
  Future<List<TrustedContactModel>> getAllTrustedContactsByUserId(String userId) async {
    final snapshot = await FirebaseQuery.getDocuments(
      collection: _collection,
      queryBuilder: (collection) =>
          collection.where('userId', isEqualTo: userId).orderBy('createdAt', descending: true),
    );
  ...
  Future<List<TrustedContactModel>> searchTrustedContactsByName({
    ...
    final snapshot = await FirebaseQuery.getDocuments(
      collection: _collection,
      queryBuilder: (collection) =>
          collection.where('userId', isEqualTo: userId).where('isActive', isEqualTo: true).orderBy('name'),
    );
  ...
  Future<void> updateTrustedContact({
    ...
    await FirebaseQuery.updateDocument(collection: _collection, docId: contactId, data: data);
  }
  ...
  Future<void> deleteTrustedContact(String contactId) async {
    await FirebaseQuery.deleteDocument(collection: _collection, docId: contactId);
  }

  // Stream - Real-time updates for a specific trusted contact
  Stream<DocumentSnapshot<Map<String, dynamic>>> trustedContactStream(String contactId) {
    return FirebaseQuery.streamDocument(collection: _collection, docId: contactId);
  }

  // Stream - Real-time updates for user's trusted contacts
  Stream<QuerySnapshot<Map<String, dynamic>>> trustedContactsStream(String userId) {
