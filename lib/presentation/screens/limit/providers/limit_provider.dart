...
class LimitProvider with ChangeNotifier {
  ...
  void initializeLimit() async {
    ...
    if (userUid != null) {
      _transactionLimit = await _userRepository.getUserLimitTransaction(uid: userUid);
      _isLoading = false;
      notifyListeners();
    }
  }
  ...
  Future<void> updateLimit() async {
    ...
    try {
      String? userUid = await _sessionUtil.readSession(_sessionUtil.userKey);
      final success = await _userRepository.updateUserLimitTransaction(uid: userUid ?? '', newLimit: _transactionLimit);
